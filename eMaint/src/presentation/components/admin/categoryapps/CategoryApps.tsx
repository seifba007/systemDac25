...
const getDateLabel = (notificationTime: string): string => {
  ...
  const isYesterday = notificationDate.getDate() === now.getDate() - 1 &&
...
const CategoryApps: React.FC<NotificationNavProps> = ({ data }) => {
  ...
  return (
    <Menu shadow="md" width={200} position="bottom-end" offset={2}>
      ...
      <Menu.Dropdown w={ '25%'}>
      <Menu.Label fw={'600'} pb={'1em'} pt={'1em'}>Corporate Apps </Menu.Label>
        ...
        <Menu.Divider />
        <Menu.Label fw={'600'} pb={'1em'} pt={'1em'}>Demo Apps
        </Menu.Label>
