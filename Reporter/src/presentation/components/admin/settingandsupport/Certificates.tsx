...
import {
	...
	DeleteCertificates,
	getCertificates,
	getUserAssignmentsbyCertificates,
	updateUserCertificate,
} from '@/core/services/modulesServices/Certificates.service';
...
const Certificates = () => {
	...
	const updateUserCertificated = async (
		userId: string,
		...
		const updateData = {
			isValid: certificateValidity,
			validationDate: validationDate,
			validityPeriod:
				certificates.find((c) => c.certificateId === certificateId)?.validityPeriod || 12,
		};
		try {
			await updateUserCertificate(updateData, certificateId, userId);
			toast.success(' updated successfully ');
			getCertificates(); // Refresh certificates
			setModalOpenIteam(false);
		} catch (error) {
	...
	return isLoading ? (
		...
		<Stack>
									...
									<Table.Td pl={'1em'}>
										<Flex gap='0.5em'>
											...
											<ActionIcon
												...
												h='20px'
												onClick={() => {
													setCertificatedataone(certificate);
											...
											<ActionIcon
												...
												h='20px'
												onClick={() => {
													getUserAssignments(certificate._id);
													setModalOpenIteam(true);
												}}
											>
											...
											<ActionIcon
												...
												onClick={() => {
													openVisibility();
													setCertificate(certificate?._id);
												}}
											>
			...
			<Paper shadow='xs' p='xl'>
				<Stack>
					...
					<FileInput
						...
						onChange={setFile}
					/>
					<Button w={'20%'} bg={'#4254ba'} onClick={parseAndUploadCertificates} loading={isLoading}>
			...
			<ApproveItem
				...
				updateUserCertificate={updateUserCertificated}
			/>
			<DeleteModal
