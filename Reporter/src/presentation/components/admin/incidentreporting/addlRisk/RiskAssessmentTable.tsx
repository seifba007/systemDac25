...
const RiskAssessmentTable: React.FC<RiskAssessmentTableProps> = ({ onRowsChange, initialRows }) => {
	...
	const handleInputChange = (index: number, field: keyof RiskAssessmentRow, value: string) => {
		...
		<div>
			...
			<ScrollArea h={200}>
				<Table withTableBorder withColumnBorders>
					...
					<Table.Tbody>
						{rows.map((row, index) => {
							const { riskLabel: initialRiskLabel, backgroundColor: initialBackgroundColor } =
								getRiskDetails(row.likelihood, row.severity);
							const { riskLabel: residualRiskLabel, backgroundColor: residualBackgroundColor } =
								getRiskDetails(row.residualLikelihood, row.residualSeverity);
								...
								<Table.Tr key={index}>
									...
									<Table.Td>
										<TextInput
											...
											onChange={(e) =>
												handleInputChange(index, 'hazardDescription', e.target.value)
											}
										/>
									</Table.Td>
									<Table.Td>
									...
									<Table.Td>
										<TextInput
											...
											onChange={(e) =>
												handleInputChange(index, 'preventionMeasures', e.target.value)
											}
										/>
									</Table.Td>
									<Table.Td>
										<TextInput
											...
											onChange={(e) =>
												handleInputChange(index, 'mitigationMeasures', e.target.value)
											}
										/>
									</Table.Td>
									<Table.Td w={'5%'}>
										<Select
											...
											onChange={(value) =>
												handleInputChange(index, 'residualLikelihood', value || '')
											}
											styles={selectStyles}
										/>
									</Table.Td>
									<Table.Td w={'5%'}>
										<Select
											...
											onChange={(value) =>
												handleInputChange(index, 'residualSeverity', value || '')
											}
											styles={selectStyles}
										/>
									</Table.Td>
