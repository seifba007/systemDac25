...
import {
	getCertificatesUserAssignments,
...
const Certification = () => {
	...
	const handleQuizSubmit = async () => {
		...
		currentQuestions.forEach((question, index) => {
			...
			console.log(
				`Question ${index + 1}: User Answer = "${userAnswer}", Correct Answer = "${correctAnswer}"`,
			);
		...
		if (score === 100) {
			if (!user?.id) {
				...
				const updateData = {
					...
					validityPeriod:
						certificates.find((c) => c.certificateId === currentCertId)?.validityPeriod || 12,
				};
			...
			<Paper shadow='xs' p='xl'>
				<ScrollArea h={350}>
					<Table highlightOnHover withTableBorder withColumnBorders>
						<Table.Thead>
							<Table.Tr>
								<Table.Th style={{ padding: '10px', fontSize: '13px' }}>
									Certification Name
								</Table.Th>
			...
			{showQuizModal && (
				<Modal
					...
					title={
						<Text c='#fff'>
							Take Quiz for{' '}
							{certificates.find((c) => c.certificateId === currentCertId)?.name || currentCertId}
						</Text>
					}
					fullScreen
					radius={0}
					...
					<Stack pt={'1em'} id='quizResultsContainer'>
						...
						{currentQuestions.map((question, index) => (
							<div key={index}>
								...
								<Text fz={'14px'} c={'#f7473a'}>
									Your answer: {userAnswers[index] || 'Not answered'}
								</Text>
								<Text fz={'14px'} c={'#17a497'}>
