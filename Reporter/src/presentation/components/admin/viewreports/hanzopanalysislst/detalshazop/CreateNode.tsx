...
const CreateNode = ({ opened, onClose, onSave }: CreateNodeProps) => {
  ...
  const addParameter = () => {
    if (newParameter.parameter && newParameter.deviation) {
      ...
      setNewNode((prev) => ({
        ...
        parameters: [
          ...prev.parameters,
          `${newParameter.parameter} (${newParameter.deviation})`,
        ],
      }));
      setNewParameter({
        parameter: "",
  ...
  const removeParameter = (index: number) => {
    ...
    setNewNode((prev) => ({
      ...prev,
      parameters: updatedParametersList.map(
        (param) => `${param.parameter} (${param.deviation})`
      ),
    }));
  };
  ...
  return (
    <Modal
      opened={opened}
      onClose={onClose}
      title={<Text fz="17px" fw={600} c="#6c757d">Node Information</Text>}
      radius={8}
                  ...
                  <td>
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        setNewNode((prev) => ({
                          ...prev,
                          parameters: updatedList.map(
                            (p) => `${p.parameter} (${p.deviation})`
                          ),
                        }));
                      }}
                      style={{
          ...
          <div className="row mb-3">
            ...
            <div className="col-md-3">
              <input
                ...
                onChange={(e) =>
                  setNewParameter({ ...newParameter, comments: e.target.value })
                }
                style={{ fontSize: "13px", padding: "8px", borderRadius: "4px" }}
              />
            </div>
            ...
            >
              ...
              <Button
                ...
                onClick={() =>
                  setNewNode({
                    ...newNode,
                    attachments: newNode.attachments.filter(
                      (_, i) => i !== index
                    ),
                  })
                }
