...
const DetailedHAZOPAnalysis: React.FC = () => {
	...
	const saveNode = (updatedNode: Node) => {
		if (editNode) {
			setNodes((prev) => prev.map((node) => (node.name === editNode.name ? updatedNode : node)));
	...
	const openRiskModal = (cell: HTMLElement, type: 'initial' | 'residual', item: HazopItem) => {
	...
	const setRiskValue = () => {
		...
		if (activeRiskCell) {
			...
			setHazopItems((prev) => prev.map((i) => (i.id === item.id ? updatedItem : i)));
		}
	...
	return (
		<Box pt={'1em'} pr={'1em'} style={{ backgroundColor: '#f2f2f7' }}>
					...
					<div className='card p-3' style={{ backgroundColor: '#ffffff', borderRadius: '8px' }}>
						...
						<div className='mb-3'>
							<Flex justify='space-between' align='center' mb='md'>
								...
								<Button size='xs' variant='filled' color='red' style={{ padding: '4px 8px' }}>
									<svg
										width='16'
										height='16'
										viewBox='0 0 24 24'
							...
							<Flex wrap={'wrap'}>
								<div className='col-md-3'>
									<label
										...
										style={{ fontSize: '10px', fontWeight: 600, color: '#6c757d' }}
									>
										* Parameter
									</label>
									<select
										...
										onChange={(e) =>
											updateHazopItem(hazopItems[0]?.id || 0, 'parameter', e.target.value)
										}
										style={{
											fontSize: '10px',
											padding: '4px 8px',
								...
								<div className='col-md-3'>
									...
									<select
										...
										onChange={(e) =>
											updateHazopItem(hazopItems[0]?.id || 0, 'deviation', e.target.value)
										}
										style={{
											fontSize: '10px',
											padding: '4px 8px',
								...
								<div className='col-md-3'>
									...
									<input
										...
										onChange={(e) =>
											updateHazopItem(hazopItems[0]?.id || 0, 'designIntent', e.target.value)
										}
										style={{
											fontSize: '10px',
											padding: '4px 8px',
								...
								<div className='col-md-3'>
									...
									<input
										...
										onChange={(e) =>
											updateHazopItem(hazopItems[0]?.id || 0, 'comments', e.target.value)
										}
										style={{
											fontSize: '10px',
											padding: '4px 8px',
						...
						<div className='mt-3'>
							<h5
								style={{ fontSize: '17px', fontWeight: 600, color: '#6c757d', marginBottom: '1em' }}
							>
								Hazop Analysis
							</h5>
								...
								>
									<thead className='border-top border-bottom bg-light-subtle border-light'>
										<tr className='grandchild-header'>
											...
											<th
												style={{ width: '4.16666667%', fontSize: '13px' }}
												...
												<tr key={item.id}>
													...
													<td>
														<input
															...
															onChange={(e) =>
																updateHazopItem(item.id, 'consequence', e.target.value)
															}
													...
													<td className='text-center'>
														<Button
															...
															title={`Likelihood: ${item.initialLikelihood || 'N/A'}, Severity: ${
																item.initialSeverity || 'N/A'
															}, Risk: ${item.initialRisk}`}
														>
															<Text fz={'12px'} c='#fff'>
																{initialRisk.level} ({item.initialRisk})
															</Text>
														</Button>
													</td>
													<td>
														<input
															...
															onChange={(e) =>
																updateHazopItem(item.id, 'safeguard', e.target.value)
															}
													...
													<td>
														<input
															...
															onChange={(e) =>
																updateHazopItem(item.id, 'recommendation', e.target.value)
															}
													...
													<td className='text-center'>
														<Button
															...
															title={`Likelihood: ${item.residualLikelihood || 'N/A'}, Severity: ${
																item.residualSeverity || 'N/A'
															}, Risk: ${item.residualRisk}`}
														>
															<Text fz={'12px'} c={'#fff'}>
																{residualRisk.level} ({item.residualRisk})
															</Text>
														</Button>
													</td>
													<td className='text-center'>
			...
			{/* CreateNode Modal */}
			<CreateNode opened={modalOpened} onClose={closeModal} onSave={saveNode} />
